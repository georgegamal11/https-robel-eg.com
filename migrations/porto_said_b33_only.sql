-- تخصيص توليد المواصفات لمشروع بورتو سعيد لمبنى B33 فقط
...
BEGIN
  ...
  SET auto_specs = 
    CASE 
      -- قواعد مشروع بورتو جولف مارينا (تظل كما هي لجميع مباني المشروع)
      ...
      -- قواعد مشروع بورتو سعيد - مخصصة لمبنى B33 فقط
...
-- 3. تنظيف البيانات: إفراغ المواصفات لأي مبنى في بورتو سعيد غير B33
UPDATE units 
SET auto_specs = NULL 
WHERE project_id = 'porto-said' AND building_id != 'B33' AND building_id != '33';

-- 4. التأكد من تحديث وحدات بورتو سعيد مبنى B33 بالمنطق الصحيح
UPDATE units
SET auto_specs = 
    CASE 
      ...
      ELSE NULL
    END
WHERE project_id = 'porto-said' AND (building_id = 'B33' OR building_id = '33');
