...
// ============================================================
async function main() {
  console.log("🚨 DATABASE WIPE & RESTORE - 222 UNITS");
  console.log("========================================\n");

  // STEP 1: DELETE ALL EXISTING DATA
  console.log("🗑️  Deleting all existing units...");
  ...
  // STEP 2: UPLOAD 222 NEW UNITS
  console.log("📥 Uploading 222 units from PDF...");
  ...
  for (const unit of units) {
    ...
    // Determine project_id from building_id
    let project_id = "porto-golf-marina";
    if (unit.building_id === "B15" || unit.building_id === "B33") {
      project_id = "porto-said";
    }
    ...
    const dataToUpload = {
      ...
      floor: unit.floor.replace(" Floor", "").replace(" floor", ""),
      view: unit.view,
      area: unit.area,
      price: unit.price,
      status: unit.status || "Available",
      purpose: unit.purpose.toLowerCase(),
      payment_plan: "",
      images: "[]",
      auto_specs: ""
    };

    process.stdout.write(`\r   Uploading ${uploaded}/222: ${unitId}...`);
  ...
  console.log("\n📊 Summary:");
  console.log(`   - Total units: 222`);
  console.log(`   - Images: Will be added later`);
  console.log(`   - Payment plans: Will be added later`);
}
