...
async function initAvailableBuildings() {
    ...
    try {
        console.log(`ðŸš€ [AvailableUnits] Syncing ${CURRENT_PAGE_AREA}...`);
        ...
        if (window.firebaseQueries) {
            ...
            if (!cfUnits || cfUnits.length === 0) {
                console.warn("âš ï¸ Initial sync returned 0 units. Retrying with FORCE REFRESH...");
                ...
                if (cfUnits && cfUnits.length > 0) {
                    ...
                    console.log(`âœ… [AvailableUnits] Retry succesful: ${localInventory.length} units synced.`);
                } else if (localInventory && localInventory.length > 0) {
                    console.warn("âŒ Retry failed but local cache exists. Keeping existing data.");
                } else {
                    console.error("âŒ Retry failed. Still 0 units and no local data.");
                ...
                localInventory = cfUnits;
                window.inventory = localInventory;
                console.log(`âœ… [AvailableUnits] Synced ${localInventory.length} units from Layer.`);
            }
            ...
            buildingsList.forEach(bId => {
                ...
                // ðŸš€ Fallback Project Area Mapping
                const getFallbackArea = (id) => {
                    ...
                    if (['15', '16', '17', '33', '9', '10', 'shops'].includes(clean)) return "Porto Said";
        ...
        console.log(`âœ… [AvailableUnits] D1 Sync Complete. Rendering ${CURRENT_PAGE_AREA}...`);
...
function getBuildingMetadataImage(pName) {
    ...
    const mapping = {
        '121': 'images/ui/logo-main.png',
        '243': 'images/ui/logo-main.png',
        'B133': 'images/ui/logo-main.png',
        'B136': 'images/ui/logo-main.png',
        'B230': 'images/ui/logo-main.png',
        '224': 'images/projects/porto-golf-marina/buildings/224.webp',
        '78': 'images/projects/porto-golf-marina/buildings/78.webp',
        'SHOPS': 'images/projects/porto-said/hero/hero-1.webp'
    };
    return mapping[pName] || 'images/projects/porto-golf-marina/main.jpg';
}
...
function renderBuildingsInternal(filter = 'all') {
    ...
    let filtered = buildingsList.filter(id => {
        ...
        if (!meta) {
            ...
            if (CURRENT_PAGE_AREA === "Porto Golf Marina" && golfIds.includes(cleanId)) {
                console.log(`âœ… [AvailableUnits] ${id} included (fallback match - no meta)`);
                return true;
            }
            console.log(`âŒ [AvailableUnits] ${id} excluded (no meta)`);
            return false;
        }
        ...
        if (!matches && CURRENT_PAGE_AREA === "Porto Golf Marina" && golfIds.includes(cleanId) && String(id).toUpperCase() !== "READY") {
            console.log(`âœ… [AvailableUnits] ${id} included (meta mismatch but fallback match)`);
            return true;
        }

        if (matches) console.log(`âœ… [AvailableUnits] ${id} included (meta match: ${normalizedMetaArea})`);
        else console.log(`âŒ [AvailableUnits] ${id} excluded (area mismatch: ${normalizedMetaArea} vs ${CURRENT_PAGE_AREA})`);

        return matches;
    });

    console.log(`ðŸš€ [AvailableUnits] Final filtered list for ${CURRENT_PAGE_AREA}:`, filtered);
    ...
    // ðŸš€ Update Sidebar Dropdowns Dynamically
    refreshSidebars(filtered);

    // ðŸš€ Refresh AOS to pick up new elements
    if (window.AOS) {
        setTimeout(() => {
            window.AOS.refresh();
        }, 300);
