...
// ðŸš€ CRITICAL: Project Metadata for Building Images (Safe Fallback)
window.projectMetadata = window.projectMetadata || {
    // Porto Golf Marina Buildings
    "B133": { projectArea: "Porto Golf Marina", image: ["images/ui/logo-main.png"] },
    "B136": { projectArea: "Porto Golf Marina", image: ["images/ui/logo-main.png"] },
    "B230": { projectArea: "Porto Golf Marina", image: ["images/ui/logo-main.png"] },
...
function setLanguage(lang) {
    console.log(`ðŸŒ [Units] Setting language to: ${lang}`);
    if (typeof translations === 'undefined') {
        console.error('âŒ Translations not loaded!');
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // ðŸš€ CACHE POLICY: Use existing cache if available, don't purge on every visit.
    console.log('â™»ï¸ System ready. Using optimized cache policy.');

    // ðŸš¨ CRITICAL FIX: Override home.js global functions
    if (typeof window.refreshFilterOptions === 'function') {
        console.warn('âš ï¸ Disabling global refreshFilterOptions() for units.html');
    ...
    try {
        ...
        const startArea = params.get('area'); // ðŸš€ NEW: Parse Area
        const startDelivery = params.get('delivery'); // ðŸš€ NEW: Parse Delivery
        ...
        if (rawProject) {
            ...
            if (pKey) {
                ...
                // ðŸš€ APPLY URL FILTERS (Post-Init)
                ...
                if (startArea) {
                    ...
                    console.log(`ðŸŽ¯ Applying Area Filter from URL: ${targetArea}`);
...
async function enterBuildingMode(projectId, buildingId) {
    ...
    console.log(`ðŸ¢ Entering Building Mode: ${normBuildingId} (${projectId})`);
    ...
    // ðŸš€ STEP 3A: Load building units from cache first, refresh in background
    console.log(`ðŸ” Fetching units for ${buildingId}...`);
    try {
        let buildingUnits = await window.firebaseQueries.getUnitsByBuilding(normBuildingId, false); // ðŸš€ Cache-first for speed
        if (!buildingUnits || buildingUnits.length === 0) {
            console.log(`  â†³ Retry without prefix: ${buildingId}`);
            buildingUnits = await window.firebaseQueries.getUnitsByBuilding(buildingId, false); // Try raw (133)
        }

        if (buildingUnits && buildingUnits.length > 0) {
            console.log(`âœ… [Step 3A] Found ${buildingUnits.length} units for building ${buildingId}`);
            ...
            console.log(`âœ… [Step 3A] Mapped ${processedUnits.length} valid units. Sample:`, processedUnits[0]);
            filterState.allProjectUnits = processedUnits;
            cacheUnits(processedUnits); // ðŸš€ Update Global Cache for comparison persistence

            // ðŸ”“ UNLOCK UI: Show basic results immediately
            populateBuildingSpecificFilters();
            applyFilters();
            hideLoader(); // Hide global loader NOW
        } else {
            console.warn(`âš ï¸ [Step 3A] No units found for building ${buildingId} (Raw/Norm)`);
    ...
    // ðŸš€ STEP 3B: Load full project data in background (silent)
    // Only overwrite if we found NO units, OR if we want to expand scope
    loadProjectData(projectId).then(() => {
        console.log('âœ… Full project data loaded in background.');

        // ðŸ›¡ï¸ RE-APPLY Building filter logic & RESTORE if lost
        if (filterState.building) {
            const hasBuildingUnits = filterState.allProjectUnits.some(u => u.building_id === filterState.building);

            if (!hasBuildingUnits) {
                console.warn(`âš ï¸ Project load wiped building units! Force-restoring building ${filterState.building}`);
                ...
                window.firebaseQueries.getUnitsByBuilding(filterState.building, false).then(recovered => {
                    if (recovered && recovered.length > 0) {
                        console.log(`â™»ï¸ Recovered ${recovered.length} units.`);
                        // Merge clean
...
/**
 * NEW: Unified Filter Option Refresher
 * Ensures Building and Area filters stay in sync with each other's selections.
 */
function refreshDropdownOptions() {
    if (!filterState.allProjectUnits || filterState.allProjectUnits.length === 0) {
        // Clear everything if no units
        if (dom.buildingFilter) dom.buildingFilter.innerHTML = '<option value="">All Buildings</option>';
        if (dom.areaFilter) dom.areaFilter.innerHTML = '<option value="">All Areas</option>';
        return;
    }

    console.log('🔄 Refreshing dependent filter options...');

    // 1. Populate Buildings based on selected Area
    populateBuildingsFilter(filterState.allProjectUnits);

    // 2. Populate Areas based on selected Building
    populateAreasFilter(filterState.allProjectUnits);
}

function populateAreasFilter(units) {
    if (!dom.areaFilter) return;

    // Filter units by current building if selected
    let relevantUnits = units;
    if (filterState.building) {
        relevantUnits = units.filter(u => u.building_id === filterState.building);
    }
    ...
    if (config.areaSystem === 'fixed') {
        ...
        if (areas.length === 0 && relevantUnits.length > 0) {
            // This case shouldn't happen if data is clean
            areas = config.areas || [];
        }
    ...
    // Restore selection or auto-clear if no longer valid
    if (filterState.area) {
        const options = Array.from(dom.areaFilter.options).map(opt => opt.value);
        if (!options.includes(filterState.area)) {
            console.warn(`⚠️ Area ${filterState.area}m² is no longer available for building ${filterState.building}, clearing...`);
            filterState.area = null;
            dom.areaFilter.value = "";
        } else {
            dom.areaFilter.value = filterState.area;
        }
    } else {
        dom.areaFilter.value = "";
    }
...
function hideLoader() {
    console.log('ðŸ Hiding page loader...');
...
function resolveProjectKey(input) {
    ...
    // ðŸš€ NEW: Smart Building ID to Project Resolver
    const buildingMap = {
        ...
        'porto-golf': 'porto-golf-marina', 'porto-golf-marina': 'porto-golf-marina'
    };
...
function setupEventListeners() {
    ...
    // ðŸš€ NEW: Enhanced Back Button (Internal Navigation)
    const backBtn = document.getElementById('back-btn');
    if (backBtn) {
        backBtn.onclick = (e) => {
            e.preventDefault();
            console.log('ðŸ”™ Going back home/project page...');
...
async function selectProject(projectId, preserveFilters = false) {
    ...
    console.log('ðŸŽ¯ Setting project scope:', projectId);
    ...
    if (dom.areaFilter) {
        dom.areaFilter.value = "";
        // ðŸš¨ CRITICAL: Clear ALL old options to prevent mixing projects
        dom.areaFilter.innerHTML = '<option value="">All Areas</option>';
    }
...
async function loadProjectData(projectId) {
    ...
    console.log(`âœ… Final Unit Count for ${projectId}:`, units ? units.length : 0);
    ...
    // ðŸš€ MERGE LOGIC: Prevent background Project-load from overwriting specific Building-load
    if (filterState.allProjectUnits && filterState.allProjectUnits.length > 0) {
        ...
        if (newOnes.length > 0) {
            console.log(`â™»ï¸ Merging ${newOnes.length} new units from project load...`);
            filterState.allProjectUnits = [...filterState.allProjectUnits, ...newOnes];
            cacheUnits(newOnes); // ðŸš€ Cache new units
        }
    } else {
        console.log(`ðŸ“¦ Storing ${processedUnits.length} units in filterState...`);
        filterState.allProjectUnits = processedUnits;
        cacheUnits(processedUnits); // ðŸš€ Initial cache
    }

    // 🚀 STEP 4: Dynamic Filter Options
    // Instead of using hardcoded areas from config, we refresh options based on loaded units.
    ...
    // ðŸš€ CRITICAL FIX: Small delay ensures DOM rendering completes before filtering
    console.log(`â³ Waiting for DOM to stabilize before applying filters...`);

    await new Promise(resolve => setTimeout(resolve, 150));

    console.log(`ðŸ” Now applying filters to ${filterState.allProjectUnits?.length || 0} loaded units...`);
    applyFilters();
}
...
function populateDeliveryButtons(units) {
    ...
    console.log('ðŸ“… Available delivery options:', deliveryStatuses);
...
function onAreaChange(val) {
    ...
    // 🚀 LINKED FILTERS: Refresh Building options when Area changes
    refreshDropdownOptions();
...
function populateBuildingsFilter(units) {
    ...
    // ðŸš€ SMART FILTER: If area is selected, only show buildings that have units with that area
    if (filterState.area) {
        console.log(`ðŸ—ï¸ Filtering buildings by area: ${filterState.area}`);
        ...
        buildings = buildings.filter(buildingId => {
            ...
            if (hasMatchingArea) {
                console.log(`  âœ… ${buildingId}: Has units with ${filterState.area}m\u00B2`);
            } else {
                console.log(`  âŒ ${buildingId}: No units with ${filterState.area}m\u00B2`);
            }

            return hasMatchingArea;
        });

        console.log(`ðŸ“Š Buildings with area ${filterState.area}: ${buildings.join(', ')}`);
    }
    ...
    // ðŸš€ AUTO-CLEAR: If current building is not in the filtered list, clear selection
    // ðŸš¨ FIX: Don't clear if we are in "Building Mode" (header visible)
    ...
    if (filterState.building && !buildings.includes(filterState.building) && !isBuildingMode) {
        console.warn(`âš ï¸ Building ${filterState.building} doesn't have matching units, clearing selection`);
        filterState.building = null;
        if (dom.buildingFilter) dom.buildingFilter.value = "";
    }
}
...
function populateAreasFixed(areas) {
    console.log('ðŸ“ populateAreasFixed called with:', areas);
    console.log('ðŸ“ Area filter element:', dom.areaFilter);

    if (!dom.areaFilter) {
        console.error('âŒ Area filter dropdown not found in DOM!');
        return;
    }
    ...
    areas.forEach(area => {
        ...
        dom.areaFilter.appendChild(opt);
        console.log(`  âœ… Added area option: ${area}mÂ²`);
    });

    console.log(`ðŸ“ Total area options: ${dom.areaFilter.options.length}`);
}

function populateAreasRanges(ranges) {
    console.log('ðŸ“ populateAreasRanges called with:', ranges);
    console.log('ðŸ“ Area filter element:', dom.areaFilter);
    ...
    ranges.forEach(range => {
        ...
        opt.textContent = range.label;
        dom.areaFilter.appendChild(opt);
        console.log(`  âœ… Added area range: ${range.label} (${range.min}-${range.max})`);
    });
...
function applyFilters() {
    ...
    console.log(`ðŸ” Starting filter with ${filtered.length} total units`);
    ...
    if (filterState.area) {
        ...
        if (filterState.projectAreaSystem === 'fixed' || filterState.building) {
            const val = Number(filterState.area);
            console.log(`[Filter] Area (fixed): Looking for exact ${val}mÂ²`);
            filtered = filtered.filter(u => {
                const match = u.area === val;
                if (!match) console.log(`  âŒ Unit ${u.code}: ${u.area}mÂ² != ${val}mÂ²`);
                return match;
            });
        } else {
            const [min, max] = filterState.area.split('-').map(Number);
            ...
            filtered = filtered.filter(u => {
                ...
                if (!match) console.log(`  âŒ Unit ${u.code}: ${u.area}m\u00B2 not in ${min}-${max}`);
                return match;
            });
        }
    ...
    if (filterState.delivery !== 'All') {
        ...
        if (filtered.length === 0 && countBefore > 0) {
            console.warn(`âš ï¸ All units were filtered out by delivery filter!`);
    ...
    console.log(`âœ… Final filtered count: ${filtered.length} units`);
...
function clearSpecificFilter(type) {
    ...
    if (type === 'area') {
        filterState.area = null;
        dom.areaFilter.value = "";
        // ðŸš€ Refresh buildings list to show all buildings again
        if (filterState.allProjectUnits && filterState.allProjectUnits.length > 0) {
            populateBuildingsFilter(filterState.allProjectUnits);
        }
    }
...
function openBuildingDetails(buildingData) {
    console.log('ðŸš€ Intercepted building click: Transitioning without reload');
...
function normalizeId(id) {
    ...
    id = id.trim().toUpperCase(); // ðŸš¨ CRITICAL: Trim spaces & Uppercase
...
function createUnitCard(u) {
    ...
    // ðŸš€ NEW: Intelligent Building-Level Fallback
    ...
    card.innerHTML = `
        <div class="unit-card-img-container relative aspect-[16/10] sm:aspect-video bg-gray-50 border-b border-gray-50">
            ...
            <div class="compare-click-area absolute top-0 left-0 p-2 sm:p-4 z-40" 
                 onclick="console.log('âœ… Click Area Hit for Unit: ${u.code || u.id}'); event.stopPropagation(); window.toggleCompare('${u.code || u.id}')">
...
window.toggleCompare = function (unitId) {
    console.log(`ðŸ”„ Toggling comparison for unit: ${unitId}`);
...
window.openCompareModal = function () {
    console.log('ðŸš€ Attempting to open comparison modal...');
    const d = getDom();
    ...
    console.log(`ðŸ“Š Matched ${selectedUnits.length} units.`);
