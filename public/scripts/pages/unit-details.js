...
const PROJECT_MASTER_PLANS = {
    'Porto Golf Marina': 'images/projects/porto-golf-marina/master-plan/overview-2.jpg',
    'Porto Said': 'images/projects/porto-said/master-plan/master plan porto said.webp',
    'Celebration': 'images/projects/celebration/master-plan/overview-main.jpg'
};
...
function normalizeUnit(u) {
    ...
    const normalized = {
        ...
        building: u.buildingCode || u.buildingId || u.building_id || u.project_id || u.project || 'Building',
        location: u.location || u.projectArea || u.projectName || rawProject || 'Porto Golf Marina',
...
function renderUnit() {
    ...
    const displayId = (u.code || u.unit_id || u.id || '---').toString().replace(/^unit_/i, '');
    const bName = u.building || 'Building';
    const displayTitle = `${bName} - ${unitLabel} ${displayId}`;

    console.log("🛠️ Rendering unit:", displayId, "in building:", bName);
    ...
    try {
        ['unit-title', 'unit-title-mobile', 'unit-title-desktop', 'unit-title-pc', 'page-main-title', 'nav-title', 'overlay-building'].forEach(id => {
            ...
            else if (id === 'overlay-building') el.textContent = bName;
            else {
                el.innerHTML = `${bName} - ${unitLabel} <span lang="en">${displayId}</span>`;
            }
        });
    } catch (e) { console.error("❌ Title Render Error:", e); }
