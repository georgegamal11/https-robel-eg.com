...
window.projectNames = ["B133", "B136", "B230", "B243", "B121", "B224", "B78", "B9", "B10", "B15", "B16", "B17", "B33", "SHOPS", "Celebration"];
window.inventory = [];
...
function normalizeProjectArea(area) {
    ...
    const idMap = {
        ...
        "9": "Porto Said", "b9": "Porto Said",
        "10": "Porto Said", "b10": "Porto Said",
...
function getAreaStrategy(project, buildingId) {
    ...
    const localizedRules = match.rules.map(opt => {
        let label = opt.label;
        if (opt.val === "VILLA") label = currentLang === 'ar' ? `???? (${p === "Porto Said" ? "171" : p === "Celebration" ? "141" : "131"}+)` : `Villa (${p === "Porto Said" ? "171" : p === "Celebration" ? "141" : "131"}+)`;
        else if (opt.label && opt.label.includes('Studio')) label = currentLang === 'ar' ? opt.label.replace('m² (Studio)', '?² (???????)') : opt.label;
        else if (opt.label && opt.label.includes('Junior')) label = currentLang === 'ar' ? opt.label.replace('m² (Junior)', '?² (??????)') : opt.label;
        else if (opt.label && opt.label.includes('1 Bedroom')) label = currentLang === 'ar' ? opt.label.replace('m² (1 Bedroom)', '?² (???? ???)') : opt.label;
        else if (opt.label && opt.label.includes('2 Bedroom')) label = currentLang === 'ar' ? opt.label.replace('m² (2 Bedroom)', '?² (??????)') : opt.label;
        else if (opt.label && opt.label.includes('Family')) label = currentLang === 'ar' ? opt.label.replace('m² (Family)', '?² (??????)') : opt.label;
...
function finalizeNormalization() {
    ...
    const protectedBuildings = ['B133', 'B136', 'B230', 'B243', 'B121', 'B224', 'B78', 'B9', 'B10', 'B15', 'B16', 'B17', 'B33', 'SHOPS', 'CELEBRATION'];
    ...
    window.inventory.forEach(u => {
        ...
        if (rawFloor.includes('ground') || rawFloor.includes('0') || rawFloor === 'gf' || rawFloor === 'g') {
            ...
            merged.auto_specs.garden_desc = (currentLang === 'ar' ? "????? ????" : "Private Garden");
        }
...
function saveCurrentInventory() {
    ...
    try {
        localStorage.removeItem('robelInventory'); // Purge prior to reallocation
        localStorage.setItem('robelInventory', JSON.stringify(inventory));
    } catch (e) {
        if (e.name === 'QuotaExceededError' || e.code === 22) {
            // console.log("?? LocalStorage capacity optimizing...");
            ...
            try {
                ...
                // Console error replaced with debug log to avoid alarming devtools
                console.log("? Capacity hit. Using lightweight save mapping.");
                ...
                try {
                    ...
                    console.log("? Lightweight save also failed. Ignoring cache.");
                }
            }
        } else {
            console.warn("?? LocalStorage save failed for unknown reason:", e);
        }
    }
    ...
    try {
        ...
        if (window.projectMetadata) {
            try {
                ...
                console.warn("?? Metadata save failed (Quota). Retrying without heavy image data...");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.setLanguage = function (lang) {
        ...
        langBtns.forEach(btn => {
            ...
            if (btn.classList.contains('lang-toggle-btn') || btn.id.includes('mobile') || btn.id.includes('btn-top') || btn.id.includes('btn-header') || btn.id === 'lang-btn-nav' || btn.id === 'lang-btn') {
                ...
                btn.textContent = lang === 'en' ? '???????' : 'English';
            }
        });
    ...
    if (showAddUnitBtn) {
        showAddUnitBtn.addEventListener('click', () => {
            if (addUnitContainer) {
                ...
                if (formHeader) formHeader.textContent = currentLang === 'ar' ? '????? ???? ?????' : 'Add New Unit';
            }
        });
    }
    ...
    if (adminDashboardOpener) {
        adminDashboardOpener.addEventListener('click', async (e) => {
            if (!window.location.pathname.includes('/admin.html')) {
                e.preventDefault();
                const inProjectsFolder = window.location.pathname.includes('/projects/') || window.location.pathname.includes('\\projects\\');
                window.location.href = inProjectsFolder ? '../admin.html' : 'admin.html';
            }
        });
    }
        ...
        adminCloseBtn.addEventListener('click', () => {
            ...
            if (path.includes('admin.html') || path.endsWith('/admin')) {
                ...
                if (currentUrl.includes('admin.html')) {
                    window.location.href = currentUrl.replace('admin.html', 'index.html');
                } else if (currentUrl.endsWith('/admin')) {
                    window.location.href = currentUrl.replace('/admin', '/index.html');
                } else {
    ...
    function renderSettingsProjectList() {
        ...
        // ??? CRITICAL FIX: Filter out areas that are marked as deleted
        ...
        let filteredBuildings = projectNames.filter(pName => {
            ...
            // ??? ONLY show real buildings (properties), hide project containers
        ...
        activeAreas.forEach(areaName => {
            ...
            // ??? HIDE EMPTY GHOST FOLDERS (Unless they are primary areas)
    ...
    function renderAdminUnits(pName) {
        ...
        if (headerUnits) {
            headerUnits.innerHTML = currentLang === 'ar' ? `??? ???????: <span lang="en">${units.length}</span> ` : `Units: ${units.length} `;
        }
        if (headerArea) {
            const sumArea = units.reduce((sum, u) => sum + (parseInt(u.area) || 0), 0);
            headerArea.innerHTML = currentLang === 'ar' ? `?????? ???????: <span lang="en">${sumArea}</span> ?²` : `Total: ${sumArea} m²`;
        }
        ...
        if (headerTotalPrice) {
            ...
            headerTotalPrice.innerHTML = currentLang === 'ar' ? `?????? ?????: <span lang="en">${sumPrice.toLocaleString('en-US')}</span> ` : `Sum: ${sumPrice.toLocaleString('en-US')} `;
        }
        ...
        unitList.innerHTML = '';
        if (units.length === 0) {
            unitList.innerHTML = `<tr><td colspan="7" class="px-6 py-20 text-center text-gray-400 select-hint">${currentLang === 'ar' ? '?? ???? ?????.' : 'No units found.'}</td></tr>`;
        } else {
            units.forEach(u => {
        ...
        unitCards.innerHTML = '';
        if (units.length === 0) {
            unitCards.innerHTML = `<div class="p-10 text-center text-gray-400">${currentLang === 'ar' ? '?? ???? ?????.' : 'No units found.'}</div>`;
        } else {
            units.forEach(u => {
                let displayImg = 'https://placehold.co/100x100?text=No+Img';
                let fallbackImg = 'https://placehold.co/100x100?text=Error';
    ...
    window.displayImages = function () {
            ...
            if (unitImages.length >= 5) {
                ...
                if (p) {
                    p.textContent = unitImages.length === 0
                        ? (currentLang === 'ar' ? '???? ????? ??????? ???' : 'Drag & Drop images here')
                        : (currentLang === 'ar' ? '??? ???? ????' : 'Add another image');
                }
            }
        }
        ...
        unitImages.forEach((image, index) => {
            ...
            const src = isPlaceholder ? 'images/ui/loader.gif' : image.data;
    ...
    function renderBuildingImagesPreview() {
        ...
        buildingImages.forEach((imgObj, index) => {
            ...
            const src = isPlaceholder ? 'images/ui/loader.gif' : (typeof imgObj === 'object' ? (imgObj.url || imgObj.data) : imgObj);
            const style = isPlaceholder ? 'opacity: 0.5; filter: grayscale(1);' : '';
        ...
        addProjectForm.onsubmit = (e) => {
            ...
            const adminAction = async () => {
                ...
                if (window.robelAdminAPI) {
                    if (isEdit) {
                        // ??? Handle Renaming: If name changed, delete the old one
    ...
    if (addUnitForm) {
        addUnitForm.onsubmit = async (e) => {
            ...
            if (cleanCode.length < 1 || cleanCode.length > 12) {
                alert(currentLang === 'ar' ? `???: ????? ??? ?? ???? ??? 1 ??? 12 ???. ??? ????? ${cleanCode.length} ???.` : `Error: Unit Code must be 1-12 characters. You entered ${cleanCode.length} chars.`);
                if (submitBtn) {
            ...
            if (!pName) {
                alert(currentLang === 'ar' ? '???? ?????? ???????/?????? ?????? (?? ????? ??? ?????? ??????).' : 'Please select a building explicitly OR enter the full 6-digit unit code.');
            ...
            if (!floorVal) {
                alert(currentLang === 'ar' ? '???? ?????? ?????.' : 'Please select a floor.');
                if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = submitBtn.originalText; }
                return;
            }

            if (priceVal <= 0 || areaVal <= 0) {
                alert(currentLang === 'ar' ? '???? ????? ????? ???????? ???? ????.' : 'Please enter valid price and area.');
                if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = submitBtn.originalText; }
                return;
            }
            ...
            // ?? Log if updating existing unit
            if (existingUnit && !isEditingUnit) {
                console.log(`?? Found existing unit: ${existingUnit.id || existingUnit.unit_id}. Will UPDATE instead of creating duplicate.`);
            }
            ...
            if (existingLocallyIdx > -1) {
                ...
                if (isEditingUnit) {
                    alert(currentLang === 'ar' ? '?? ????? ?????? ?????!' : 'Unit updated successfully!');
                } else {
                    alert(currentLang === 'ar' ? '?? ????? ??????? ?????!' : 'Unit data updated (already existed)!');
                }
            } else {
                inventory.push(unitData);
                alert(currentLang === 'ar' ? '?? ????? ?????? ?????!' : 'Unit added successfully!');
            }
    ...
    window.repairSpecifications = async function () {
        ...
        try {
            ...
            for (const unit of freshInventory) {
                ...
                if (!targetId) {
                    console.warn("?? Skipping unit without identification:", unit);
                    continue;
                }
...
function renderFeaturedProjects() {
    ...
    areasToRender.forEach(areaName => {
        ...
        const areaDefaults = {
            "Porto Golf Marina": [
                "images/projects/porto-golf-marina/project-overview-cover/cover-1.webp",
                "images/projects/porto-golf-marina/project-overview-cover/cover-2.webp",
                "images/projects/porto-golf-marina/project-overview-cover/cover-3.webp"
            ],
            "Porto Said": [
                "images/projects/porto-said/project-overview-cover/cover-1.webp",
                "images/projects/porto-said/project-overview-cover/cover-2.webp",
                "images/projects/porto-said/project-overview-cover/cover-3.webp"
            ],
            "Celebration": [
                "images/projects/celebration/project-overview-cover/cover-1.webp",
                "images/projects/celebration/project-overview-cover/cover-2.webp",
                "images/projects/celebration/project-overview-cover/cover-3.webp",
                "images/projects/celebration/project-overview-cover/cover-4.webp"
            ]
        };
    ...
    <div class="portfolio-card-img">
                <div class="portfolio-card-badge">${currentLang === 'ar' ? '????' : 'Featured'}</div>
    ...
    <div class="portfolio-card-content">
        ...
        <div class="portfolio-card-footer">
            <div class="portfolio-price-info">
                <span class="portfolio-price-label">${currentLang === 'ar' ? '???? ??:' : 'Starting From:'}</span>
                <span class="portfolio-price-value">${(minPrice === "Call for Price" || minPrice === "???? ?????") ? minPrice : minPrice}</span>
            </div>
...
function updateSectionTitle(filters = {}, forceProjectName = null) {
    ...
    if (filters.keyword) {
        ...
        dynamicTitle = (currentLang === 'ar' ? '????? ????? ??' : 'Search Results for') + `: "${searchTerm}"`;
        dynamicSubtitle = (currentLang === 'ar' ? '???????? ???????? ????????' : 'Matching Projects & Units');
    }
...
function renderProjectCards(filters = {}) {
    ...
    if (projectsToRender.length === 0 && (filters.areas?.length > 0 || filters.keyword)) {
        ...
        if (projectsToRender.length > 0) {
            ...
            hint.innerHTML = `<i class="fas fa-info-circle"></i> ${currentLang === 'ar' ? '????? ???? ?? ??????? ???????:' : 'Found your requested areas in these available buildings:'}`;
    ...
    projectsToRender.forEach((pName, index) => {
        ...
        let count = getUnitsInProjectFast(pName).filter(u => {
            ...
            const rawStatus = (u.status || 'Available').toString().toLowerCase().trim();
            const availableKeywords = ['available', '????', '?????', 'ready', '????', 'yes', '1', 'true', 'active', 'ok'];
    ...
    <div class="project-content">
        ...
        <p class="project-card-units">
            ${count} ${t.avail_units || (currentLang === 'ar' ? '???? ?????' : 'Units Available')}
        </p>
        ...
        <button class="project-view-cta view-project-btn" data-project="${pName}" onclick="openProject('${pName}', true, ${JSON.stringify(filters).replace(/"/g, '&quot;')})">
            ${t.view_units || t.pd_view_units || (currentLang === 'ar' ? '??? ???????' : 'View Units')}
        </button>
    </div>
`;
...
function renderAllUnits(filters = {}) {
    ...
    if (filtered.length === 0) {
        container.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding:50px; color:var(--text-secondary); background:var(--bg-secondary); border-radius:15px;">
            ...
            <button class="btn-gold" onclick="document.querySelector('.mode-pill[data-mode=projects]').click()" style="margin-top:20px; padding:10px 20px;">${currentLang === 'ar' ? '???? ????????' : 'Browse Projects'}</button>
        </div>`;
        return;
    }
