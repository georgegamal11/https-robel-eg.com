...
async function handleManualUnitSubmit(e) {
    ...
    if (!unitData.paymentPlan && typeof window.getMatchedOfferForUnit === 'function') {
        const autoPlan = window.getMatchedOfferForUnit(unitData);
        if (autoPlan) {
            unitData.paymentPlan = autoPlan;
            console.log(`?? Auto-assigned Payment Plan: ${autoPlan}`);
        }
    }
