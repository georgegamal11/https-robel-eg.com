...
async function syncToCloudflare(table, action, id, data = {}) {
    try {
        console.log(`â˜ï¸ [Cloudflare Sync] START: ${table} ${action} ${id}`);

        // ðŸ”’ SECURITY LOCK: Reporters cannot modify or delete data
        const isReporter = localStorage.getItem('isReporter') === 'true';
        if (isReporter && (action === 'DELETE' || action === 'UPSERT' || action === 'UPLOAD')) {
            console.error("âŒ SECURITY LOCK: Action blocked. Reporters are restricted to 'ReadOnly/Export' access.");
        ...
        if (action === 'DELETE') {
            ...
            if (!isAdmin) {
                console.error("âŒ SECURITY LOCK: Action blocked. Only Admin can delete data.");
        ...
        // ðŸš€ DATABASE COLUMN MAPPING (CamelCase -> Underscores for D1)
        if (table === 'units') {
        ...
        if (resp.ok) {
            ...
            console.log(`âœ… Cloudflare Sync Success: ${table} ${id}`, result);
            return result;
        } else {
            ...
            console.warn(`âš ï¸ Cloudflare Sync Failed for ${table}:`, errorDetail);
            return { success: false, error: errorDetail };
        }
    } catch (e) {
        console.error(`âŒ Cloudflare Sync Network/Logic Error:`, e);
...
async function uploadToCloudflare(fileData, fileName, type = 'unit') {
    try {
        console.log(`ðŸ“¤ [Cloudflare Storage] Uploading ${fileName}...`);
        ...
        if (resp.ok) {
            const result = await resp.json();
            console.log(`âœ… Cloudflare Storage Success: ${result.url}`);
            return result.url;
        } else {
            console.warn(`âš ï¸ Cloudflare Storage Failed:`, await resp.text());
            return null;
        }
    } catch (e) {
        console.error(`âŒ Cloudflare Storage Error:`, e);
...
async function logActivity(action, details, user = 'Admin') {
    console.log(`ðŸ“ [Activity Log] ${action}`, details, user);
...
async function createUnit(unitData) {
    try {
        ...
        // ðŸ” CRITICAL: Search for existing unit by code + building (prevents duplicates from different ID formats)
        ...
        try {
            ...
            if (existing) {
                ...
                console.log(`â™»ï¸ Found existing unit: ${existingUnitId} (will UPDATE instead of creating duplicate)`);
            }
        } catch (e) {
            console.warn('âš ï¸ Could not check for existing units (timeout or error):', e.message);
        }
        ...
        const newUnit = {
            unit_id: unitId,
            code: unitData.code,
            building_id: bCode, // âœ… Use normalized building code
        ...
        // --- ðŸŽ¯ UNIFIED SYSTEM SPECIFICATIONS (STRICT RULES) ---
        ...
        console.log(`ðŸ“¤ Sending Unit ${unitId} to Cloudflare (Images: ${newUnit.images.length})`);
...
async function batchUpdateUnitStatus(unitIds, newStatus) {
    try {
        console.log(`ðŸš€ Batch updating ${unitIds.length} units to ${newStatus}...`);
        ...
        console.log(`âœ… Batch updated ${unitIds.length} units to ${newStatus}`);
...
async function deduplicateUnits() {
    try {
        console.log("ðŸ§¹ Starting Unit Deduplication...");
        ...
        Object.values(groups).forEach(group => {
            if (group.length > 1) {
                ...
                console.log(`â™»ï¸  Duplicate Found: [Building: ${winner.building_id}, Code: ${winner.code}]. Keeping ID: ${winner.unit_id || winner.id} (Has Images: ${parseImgs(winner.images).length > 0}). Deleting ${losers.length} duplicate(s).`);
            }
        });
...
async function standardizeUnitIDs() {
    try {
        console.log("ðŸ”§ Starting Unit ID Standardization...");
        const response = await fetch(`${window.CLOUDFLARE_WORKER_URL}/api/units`);
        if (!response.ok) throw new Error("Failed to fetch units");
        const units = await response.json();
        ...
        if (toUpdate.length === 0) {
            alert("âœ… All unit IDs are already standardized!");
            return;
        }

        const msg = `Found ${toUpdate.length} units with non-standard IDs.\n\nExamples:\n` +
            toUpdate.slice(0, 3).map(item => `â€¢ ${item.oldId} â†’ ${item.newId}`).join('\n') +
...
async function fixUnitPurposes() {
    try {
        console.log("ðŸ”§ Starting Unit Purpose Fix (Standardizing to 'Buy')...");
        const response = await fetch(`${window.CLOUDFLARE_WORKER_URL}/api/units`);
        if (!response.ok) throw new Error("Failed to fetch units");
        ...
        if (toFix.length > 0) {
            console.log(`âš ï¸ STARTING CRITICAL DATABASE FIX: ${toFix.length} units found with legacy labels.`);
            ...
            for (let i = 0; i < toFix.length; i += chunkSize) {
                ...
                console.log(`âœ… Processed ${processed} / ${toFix.length}`);
            }
            ...
            console.log("âœ… Database Integrity Verified: All units are correctly set to 'Buy'.");
        }
    } catch (e) {
...
async function repairSpecifications() {
    try {
        console.log("ðŸ› ï¸ Starting Specifications Repair...");
        const response = await fetch(`${window.CLOUDFLARE_WORKER_URL}/api/units`);
        if (!response.ok) throw new Error("Failed to fetch units");
        const units = await response.json();
        ...
        if (toFix.length > 0) {
            console.log(`âš ï¸ REPAIRING ${toFix.length} units with NULL specifications...`);
            ...
            console.log("âœ… All units have valid specifications.");
        }
    } catch (e) {
...
async function wipeAllUnits() {
    ...
    try {
        console.log("âš ï¸ TRIGGERING GLOBAL UNIT WIPE...");
        ...
        if (resp.ok) {
            ...
            alert("âœ… Database Wiped Successfully!");
            window.location.reload();
            return true;
        } else {
            const err = await resp.text();
            console.error("âŒ WIPE FAILED:", err);
            alert("âŒ Wipe Failed: " + err);
            if (btn) {
                btn.disabled = false;
        ...
        console.error("âŒ WIPE ERROR:", e);
...
//             console.log("ðŸ›¡ï¸ Admin Context Detected: Running Database Integrity Checks...");
...
if (typeof window !== 'undefined') {
    ...
    // console.log('âœ… Admin API (Cloudflare) loaded with Maintenance Tools');
}
