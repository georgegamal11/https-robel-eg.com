...
var AdminUI = {
    ...
    init: function () {
        ...
        // console.log('ðŸ—ï¸ AdminUI: Initializing modals and listeners...');
        try {
            ...
            // console.log('âœ… Admin UI Controller Initialized');
        } catch (e) {
            // console.error('âŒ AdminUI Initialization failed:', e);
        }
    ...
    promptDelete: function (id, type) {
        ...
        console.log(`ðŸ—‘ï¸ Prompting delete for ${type}: ${id}`);
    ...
    confirmDelete: async function () {
        ...
        try {
            ...
            console.log(`ðŸ”¥ Executing delete for type: ${type}, ID: ${id}`);
            ...
            if (type === 'unit') {
                ...
                if (hasNames && hasMeta && hasAdmin) {
                    ...
                    try {
                        console.log(`ðŸ—‘ï¸ Attempting to delete Project record: ${aRealId}`);
            ...
            if (isPermission || isOffline) {
                ...
                console.warn(`âš ï¸ ${errorType}: Performing local soft delete only.`);
    ...
    refreshGlobalLists: async function () {
        console.log('ðŸ”„ AdminUI: Refreshing UI after changes...');
        try {
    ...
    cleanupCloudflareDuplicates: async function () {
        ...
        try {
            ...
            if (toDeleteIds.length === 0) {
                this.showToast("âœ¨ Cloudflare is already clean!", "success");
                return;
            }
            ...
            this.showToast(`âœ… Cloudflare Cleanup Complete!`, "success");
    ...
    switchToPermissions: function () {
        // ðŸ” SECURITY GATE: Only admin can access this panel
    ...
    fetchUsers: async function () {
        ...
        console.log('ðŸ”‘ Token present:', !!token, '| Token value:', token);
        ...
        try {
            ...
            console.log('ðŸ“¡ /api/auth/users status:', resp.status);
            ...
            if (resp.ok) {
                ...
                console.error('âŒ Server error:', errData);
    ...
    showSmartReports: function () {
        document.getElementById('admin-settings-view').style.display = 'none';
        document.getElementById('admin-reports-view').style.display = 'block';
        if (window.SmartReport && typeof window.SmartReport.init === 'function') {
            window.SmartReport.init();
        }
    },
};
